...
const schema = z.object({ leadId: z.string().min(1), outcome: z.enum(["CONTINUE", "CLOSED_WON", "CLOSED_LOST", "NOT_INTERESTED"]), notes: z.string().optional().default(""), nextReachBackDate: z.coerce.date().optional(), nextReachBackTimeZone: z.string().optional().default("America/New_York") });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const item = await FollowUp.create({ lead: input.leadId, actor: user.sub, outcome: input.outcome, comment: input.notes, previousReachBackDate: lead.reachBackDate ?? null, nextReachBackDate: input.nextReachBackDate, nextReachBackTimeZone: input.nextReachBackTimeZone });
    ...
    lead.status = input.outcome === "CONTINUE" ? "ACTIVE" : input.outcome;
    await lead.save();
