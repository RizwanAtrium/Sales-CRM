...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<{ search?: string; stage?: string; agent?: string; closer?: string }> }) {
  ...
  const visibleLeads = leads.filter((lead) => {
    ...
    const matchesCloser = !query.closer || lead.teamLeadCloser === query.closer;
    return matchesSearch && matchesStage && matchesAgent && matchesCloser;
  });

  const teamLeadClosers = Array.from(new Set(leads.map((lead) => lead.teamLeadCloser)));
    ...
    <AnimatedPage>
      ...
      <div className="flex flex-col gap-3 rounded-xl border bg-card/90 p-4 lg:flex-row lg:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <DropdownMenu><DropdownMenuTrigger render={<Button variant="outline" />}><Users /> Team Lead / Closer</DropdownMenuTrigger><DropdownMenuContent>{teamLeadClosers.map((closer) => <DropdownMenuItem key={closer} render={<Link href={`/leads?closer=${encodeURIComponent(closer)}`} />}>{closer}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu>
            ...
            <TableBody>
              {visibleLeads.map((lead) => (
                <TableRow key={lead.id} className="h-16">
                  ...
                  <TableCell className="text-sm">{lead.teamLeadCloser}</TableCell>
