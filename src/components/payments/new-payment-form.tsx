...
export function NewPaymentForm() {
  ...
  useEffect(() => {
    ...
    fetch("/api/opportunities")
      .then((response) => response.ok ? response.json() : Promise.reject())
      .then((data) => setOpportunities((data.items ?? []).filter((item: { stage: string }) => item.stage === "APPROVED_WON" || item.stage === "CLOSED_WON" || item.stage === "FORWARDED_TO_CST")))
  ...
  return <form onSubmit={submit} className="mx-auto max-w-4xl"><Card className="bg-card/90"><CardHeader><CardTitle>Record sale payment</CardTitle><CardDescription>Capture customer, service, payment, agent, and closer details in one immutable sale log.</CardDescription></CardHeader><CardContent className="space-y-6"><div className="space-y-2"><Label htmlFor="opportunityId">Approved-Won deal</Label><select id="opportunityId" name="opportunityId" required value={selectedOpportunityId} onChange={(event) => { const nextId = event.target.value; setSelectedOpportunityId(nextId); const nextOpportunity = opportunities.find((item) => item._id === nextId); if (nextOpportunity?.setter?._id) setAgentId(nextOpportunity.setter._id); if (nextOpportunity?.closer?._id) setCloserId(nextOpportunity.closer._id); }} className="h-10 w-full rounded-lg border bg-background px-3 text-sm"><option value="">Select a real Approved-Won deal</option>{opportunities.map((item) => <option value={item._id} key={item._id}>{item.lead?.businessName ?? "Approved-Won deal"} · Outstanding ${Number(item.amountToReceive ?? item.totalDealValue).toLocaleString()}</option>)}</select></div>
