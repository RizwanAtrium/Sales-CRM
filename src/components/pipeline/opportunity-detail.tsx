...
export function OpportunityDetail({ initialOpportunity }: { initialOpportunity: OpportunityData }) {
  ...
  const [paymentReceived, setPaymentReceived] = useState(opportunity.stage === "Closed Won" ? 12400 : 0);
  ...
  async function handoff(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (/^[0-9a-f]{24}$/i.test(opportunity.id)) {
      const response = await fetch(`/api/opportunities/${opportunity.id}/handoff`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cstHandlerId: form.get("cstHandlerId") || undefined, workStartDate: form.get("workStartDate") }) });
  ...
  return (
    <>
      <div className="flex flex-wrap gap-2">
        ...
        {opportunity.stage === "In Progress" ? <><Dialog><DialogTrigger render={<Button />}><CheckCircle2 /> Close won</DialogTrigger><DialogContent><DialogHeader><DialogTitle>Close deal as won</DialogTitle><DialogDescription>Select sold services and record their contract values.</DialogDescription></DialogHeader><div className="space-y-3">{services.slice(0, 4).map((service, index) => <label key={service} className="grid grid-cols-[auto_1fr_120px] items-center gap-3 rounded-xl border p-3"><input type="checkbox" defaultChecked={index < 2} /><span className="text-sm">{service}</span><Input type="number" defaultValue={index === 0 ? 2500 : index === 1 ? 750 : 0} /></label>)}</div><DialogFooter><Button onClick={() => changeStage("Closed Won", { serviceLines: [{ serviceName: "Website", price: 2500 }, { serviceName: "SEO", price: 2000 }] })}>Confirm won deal</Button></DialogFooter></DialogContent></Dialog><Button variant="destructive" onClick={() => changeStage("Closed Lost")}><ThumbsDown /> Close lost</Button></> : null}
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
        <Card className="bg-card/90"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="text-2xl">{opportunity.business}</CardTitle><CardDescription>{opportunity.contact} · {opportunity.id}</CardDescription></div><Badge>{opportunity.stage}</Badge></div></CardHeader><CardContent className="space-y-6">
          <div className="grid gap-3 sm:grid-cols-4"><Info label="Deal value" value={opportunity.value} /><Info label="Closer" value={opportunity.owner} /><Info label="Age" value={opportunity.age} /><Info label="Probability" value={opportunity.stage === "Closed Won" ? "100%" : "68%"} /></div>
        ...
        <div className="space-y-4">
          ...
          <Card className="border-primary/15 bg-primary/[0.04]"><CardHeader><CardTitle className="text-base">CST handoff</CardTitle><CardDescription>Available after Closed-Won and Paid-in-Full.</CardDescription></CardHeader><CardContent><form onSubmit={handoff} className="space-y-3"><div className="space-y-2"><Label>CST handler</Label><select name="cstHandlerId" className="h-9 w-full rounded-lg border bg-background px-3 text-sm"><option value="">CST Manager queue</option><option value="">Sarah Khan · 11 clients</option><option value="">Nadia Ali · 8 clients</option></select></div><div className="space-y-2"><Label>Work start date</Label><Input name="workStartDate" type="date" required defaultValue="2026-06-23" /></div><Button className="w-full" type="submit" disabled={!paid || opportunity.stage !== "Closed Won"}><Send /> Forward to CST</Button></form></CardContent></Card>
        </div>
      </div>
    </>
  );
